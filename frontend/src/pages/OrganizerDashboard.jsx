    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        {[
          ...
          { name: 'date',     label: 'Début *',       type: 'datetime-local', required: true },
          { name: 'end_date', label: 'Fin',           type: 'datetime-local', required: false },
          { name: 'capacity', label: 'Capacité *',    type: 'number',        required: true, min: 1 },
          { name: 'price',    label: 'Prix (€)',      type: 'number',        required: false, min: 0, step: '0.01' },
        ].map(({ name, label, type, required, ...rest }) => (
          <div key={name} style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            <span style={fieldLabel()}>{label}</span>
            <input name={name} type={type} value={values[name]} onChange={onChange}
              required={required} style={inputStyle} {...rest} />
          </div>
        ))}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', gridColumn: '1 / -1' }}>
          <span style={fieldLabel()}>Image de couverture</span>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        <button onClick={() => toggleRegistrants(ev)} style={{
                          ...btnOutline('#6366f1'),
